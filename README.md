...
- `votes.html`은 GitHub에 저장된 캐시 스냅샷(`data/leaderboard.html`)만 읽으므로 CORS 문제 없이 동작합니다.
- 스냅샷은 `.github/workflows/update-leaderboard.yml`가 매시간 갱신하려고 시도합니다. 필요 시 `example.py`나 수동 다운로드로 `data/leaderboard.html`을 업데이트한 뒤 `git push`하면 즉시 반영됩니다.
